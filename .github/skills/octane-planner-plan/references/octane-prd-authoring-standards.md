# Product Requirements Document (PRD) Authoring Standards

Content creation standards and requirements for `.prd.md` documents.
...
[Justify that the implementation plan is the minimum necessary to satisfy the requirements. This section guards against over-engineering — adding abstractions, flexibility, or features that were not explicitly requested.]

- **Scope justification**: Every EPIC below MUST trace to at least one requirement (REQ-, SEC-, CON-, GUD-, PAT-). List any EPIC that exists for purely structural or enabling reasons and explain why it cannot be folded into an adjacent EPIC.
- **Abstractions check**: List any new abstractions (interfaces, base classes, factories, strategy patterns) introduced by this plan. For each, explain why a simpler inline approach would not suffice.
- **Configuration check**: List any new configuration options, feature flags, or extension points introduced. For each, explain the concrete requirement that demands it.
- **Could this be simpler?**: In one paragraph, describe the simplest possible approach that was considered and rejected, and explain why the proposed approach is necessary despite being more complex.
