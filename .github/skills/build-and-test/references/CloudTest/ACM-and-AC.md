...
**Important:** Do not append any additional arguments beyond those resolved from `Args`.
...
- Contains a directory named exactly: `Taef.TestAdapter.<version>`, where `<version>` is a version string (for example, 10.84.230927001)
- The path matches the pattern: `*\Taef.TestAdapter.<version>\*\TE.TestAdapter.dll`
- Does **not** include `.pkgref`
- Exclude any path where the version is represented as a separate directory segment (for example, `*\taef.testadapter\<version>\*` or `*\microsoft.taef\<version>\*`)
- If multiple matching paths remain, select the one with the **highest version number**
...
### Failure Handling
- **Report and Stop on Unrelated Failures**: Do not attempt to fix unrelated code. Report the issue and provide concrete remediation guidance (see **Infrastructure Issue Suggestions**).
