...
- **Conductor skill** — the `conductor` shared skill must be installed. Conductor is the multi-agent orchestration engine that coordinates the reviewer agents. Verify it is available with `conductor --version` (**conductor ≥ v0.1.19** is required — the reviewer agents use per-agent `context_tier`, added in conductor#252). See `~/.copilot/skills/conductor/SKILL.md` for setup.
...
The `/octane-doc-review` slash command is available when the skill is installed via Octane (`octane install --scenario doc-reviewer`). Trailing text after the document path is mapped to the `focus` input. For advanced parameters (`max_rounds`, `exclude`), use the Conductor CLI.
...
**Set `$WORKFLOW` to the absolute path of the installed plugin's workflow YAML before invoking, and run conductor from your target repository root** so `--workspace-instructions` discovers *your* repo's conventions (`AGENTS.md`, `.github/copilot-instructions.md`, `CLAUDE.md`). Octane plugins install under `$HOME/.copilot/installed-plugins/octane/`.
