...
The Living Documentation scenario leverages multiple enterprise knowledge sources to enrich documentation with context that cannot be found in code alone:

| Source | MCP Server | What It Provides |
|--------|-----------|------------------|
| Code, repos | `code-search` | Architecture, components, dependencies |
| Eng.ms docs, TSGs, runbooks | `enghub` | Engineering docs, policies, service tree, onboarding guides |
...
- **EngineeringHub MCP server** (optional) — for internal engineering docs, TSGs, runbooks, and service tree metadata from eng.ms. Install via [enghub-mcp-server-tools](https://github.com/azure-core/enghub-mcp-server-tools). Requires VPN and Microsoft corpnet access. If unavailable, documentation will be generated without internal engineering knowledge.
