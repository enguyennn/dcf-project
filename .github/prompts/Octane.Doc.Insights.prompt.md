...
3. **Pre-load Enterprise Context**  
   Before interviewing, use `enghub/*` and `work-iq/*` tools to gather background context:
   - Search eng.ms for existing TSGs, runbooks, and architecture docs related to `${input:Name}`
   ...
   Use this context to ask **more targeted follow-up questions** during the interview. For example, if an eng.ms TSG documents a known failure mode, ask whether the team has encountered it and what they learned. If a design doc mentions a trade-off, ask the interviewee to elaborate on their experience with that decision in practice.
