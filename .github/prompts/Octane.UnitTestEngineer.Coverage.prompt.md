...
- `${file}` (string, required): Path to the source file or folder to analyze coverage for. Automatically detected from the active editor.
...
Analyze code coverage for `${file}` and generate targeted tests to fill coverage gaps. Iterate until the configured coverage target (`${config:unit-test-generation.coverage-target}`, default: 80%) is met or `${config:unit-test-generation.max-coverage-iterations}` (default: 5) improvement rounds have been exhausted.
...
- `timeout_ms`: `${config:unit-test-generation.coverage-timeout-ms}` (default: 600000)
