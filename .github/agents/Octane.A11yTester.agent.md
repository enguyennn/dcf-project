...
3. **Provide Step-by-Step Evidence**
...
The frontmatter declares `tools: ['read', 'edit', 'search', 'playwright/*', 'todo']`. Each is used as follows:

| Tool | Purpose |
|------|---------|
| `read` | Read skill files and report templates referenced by the workflow. |
| `edit` | Save the comprehensive test report to disk when the user requests it (per the `octane-a11ytester-run` skill's save gate). |
| `search` | Locate skill, template, and reference files within the scenario. |
| `playwright/*` | Browser automation via the **Playwright MCP server** — navigate, interact, and capture accessibility data from web pages. |
| `todo` | Track per-step completion of the multi-test workflow (referenced by the Step Compliance Rules above). |
...
- Structure all reports using the templates in the [`report-templates`](../skills/report-templates/SKILL.md) skill (invoked from [`octane-a11ytester-run`](../skills/octane-a11ytester-run/SKILL.md))
