...
| ITEM-025 | Wire `hybridParser` into the Input step component: replace direct `parsePlainText` call with `hybridParse`. Handle async nature with loading state. On success, dispatch `AI_COMPLETE` action with parsed assumptions + metadata. | Deferred → EPIC-007 | src/components/InputStep.tsx | InputStep.tsx, useReducer workflow, and AI_COMPLETE action are EPIC-007/008 deliverables; wiring deferred to avoid throwaway code. |
